...
import { approveComment, createComment, getComments, getPendingComments, getUserComments, searchComments, updateComment } from '../../services/apiService/commentService';

const commentApiController = express.Router();
...
commentApiController.post('/create', verifyToken, checkRole(eRoles.User), async (req: AuthRequest, res) => {
...
commentApiController.put('/update/:id', verifyToken, async (req: AuthRequest, res) => {
    const user = req.user as JwtPayload;
...
commentApiController.patch('/approve/:id', verifyToken, checkRole(eRoles.Admin), async (req: AuthRequest, res) => {
    const user = req.user as JwtPayload;
    const commentId = req.params.id;
...
commentApiController.get('/list', verifyToken, async (req: AuthRequest, res) => {
...
commentApiController.get('/my-comments', verifyToken, async (req: AuthRequest, res) => {
    const user = req.user as JwtPayload;
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 10;
...
commentApiController.get('/pending', verifyToken, checkRole(eRoles.Admin), async (req: AuthRequest, res) => {
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 10;
...
commentApiController.get('/search', verifyToken, async (req: AuthRequest, res) => {
...
commentApiController.get('/user/:userId', verifyToken, checkRole(eRoles.Admin, eRoles.Customer), async (req: AuthRequest, res) => {
